...
enum States { readyToResubscribe, successfullyResubscribed, error }
...
class InvalidSubscriptionDialogState
  ...
  States currentState = States.readyToResubscribe;
  ...
  Widget build(BuildContext context) {
    ...
    var myPlaceProvider = ref.read(myPlacesProvider.notifier);
    ...
    Future<void> onReSubscribe() async {
      ...
      List<Place> oldPlaces = myPlaceProvider.places;
      List<Place> updatedPlaces = [];
      for (Place place in oldPlaces) {
        if (place.isExpired) {
      ...
      myPlaceProvider.set(updatedPlaces);
      setState(() {
        currentState = States.successfullyResubscribed;
      });
    ...
    switch (currentState) {
      case States.readyToResubscribe:
      ...
      case States.error:
        ...
        break;
      case States.successfullyResubscribed:
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text("${localizations.invalid_subscription_dialog_error_status_code} "
            "${failedToResubscribe.statusCode.toString()}"),
        Text("${localizations.invalid_subscription_dialog_error_message} "
