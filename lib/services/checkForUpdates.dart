...
Future<String> checkForUpdates()  async {
  ...
  try {
    var latestGithubReleaseUrl =
    Uri.parse('https://api.github.com/repos/nucleus-ffm/foss_warn/releases/latest');
    ...
    if (response.statusCode == 200) {
      ...
      int secondDotGithub = data['tag_name'].toString().indexOf(".", firstDotGithub+1);

      int majorReleaseVersionGithub = int.parse(data['tag_name'].toString().substring(0, firstDotGithub));
      int minorReleaseVersionGithub = int.parse(data['tag_name'].toString().substring(firstDotGithub+1, secondDotGithub));
      int patchLevelGithub = int.parse(data['tag_name'].toString().substring(secondDotGithub+1, data['tag_name'].toString().length));
      ...
      int minorReleaseVersion = int.parse(versionNumber.substring(firstDot+1, secondDot));
      int patchLevel = int.parse(versionNumber.substring(secondDot+1, versionNumber.length));
      ...
      if(data['tag_name'] == versionNumber) {
        ...
        //new major Release
        print("new major Release available $versionNumber -> ${data['tag_name']}");
        ...
        // new minor Release
        print("new minor Releaseavailable $versionNumber -> ${data['tag_name']}");
        ...
        print("new patchLevel Release available $versionNumber -> ${data['tag_name']}");
